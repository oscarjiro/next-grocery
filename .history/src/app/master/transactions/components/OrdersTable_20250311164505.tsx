...
export default function OrdersTable<T extends OrderType>({
  data,
  tableName,
  dynamicColumns,
  ...
  console.log('Orders Data:', orders) // Cek isi orders
  console.log('Order Items Data:', orderItems) // Jika ada order items
          ...
          <tbody>
            {currentPageRows.length === 0 ? (
              ...
              currentPageRows.map(row => {
                const order = row.original as T
                const orderId = order.id || 'order' // Gunakan ID order
                const productId = order.order_items?.map(item => item.products.name).join('-') || 'product' // Gabungkan nama produk sebagai unique key

                console.log('Row Key:', `${orderId}-${productId}`)
