...
const CartTable = ({ orders }: CartTableProps) => {
  ...
  const imageColumn = useMemo<ColumnDef<any>>(
    () => ({
      id: 'image_src',
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center'>
            <Image src={src} alt='Product Image' width={80} height={80} style={{ borderRadius: 8 }} />
          </div>
        )
      }
    }),
                  ...
                  <TableBody>
                    ...
                    {order.order_items?.map((item, index) => (
                      <TableRow key={`${order.id}-${index}`}>
                        {/* Gambar Produk */}
                        <TableCell width={100}>
                          <Image src='' alt={item.products.name} width={80} height={80} style={{ borderRadius: 8 }} />
                        </TableCell>
