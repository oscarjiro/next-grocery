...
export async function addToCart(userId: string, items: CheckoutItem[]) {
  const supabase = await createClient()
  ...
  try {
    ...
    if (cartCheckError && cartCheckError.code !== 'PGRST116') {
      throw new Error(cartCheckError.message) // Error selain "not found"
    }
    ...
    if (!cartId) {
      ...
      cartId = newCart.id // Gunakan ID cart baru
    }

    if (!cartId) throw new Error('Cart ID is missing!')

    // Insert items ke cart_items
    ...
    throw new Error(`[addToCart] ${error.message}`)
  }
}
