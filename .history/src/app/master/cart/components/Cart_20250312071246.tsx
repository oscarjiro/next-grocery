...
export default function Cart({ initialData, userId }: CartProps) {
  ...
  const handleCheckout = async () => {
    console.log('Selected Items:', selectedRows) // Debugging
    startTransition(async () => {
          ...
          dynamicColumns={[
            { accessorKey: 'name', header: 'Product Name' },
            {
              ...
              cell: ({ row }) => {
                ...
                return (
                  <div className='flex items-center border border-gray-400 rounded-md px-2 py-1 w-[100px]'>
                    <Button
                      variant='outlined'
                      size='small'
                      className='min-w-0 px-2 py-1 border-0 text-gray-700'
